...
public class C2027 {
    ...
    public static long dfs(long u, HashMap<Long, List<Long>> adj, HashSet<Long> visited, HashMap<Long, Long> cache) {
        if (cache.containsKey(u))
            return cache.get(u);
        visited.add(u);
        long res = u;
        if (adj.get(u) != null) {
            for (Long c : adj.get(u)) {
                long v = u + c;
                if (!visited.contains(v)) {
                    res = max(res, dfs(v, adj, visited, cache));
                }
            }
        }
        cache.put(u, res);
    ...
    public static void helper() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            long sum = nums[i] + i;
            adj.computeIfAbsent(sum, k -> new ArrayList<>()).add((long) i);
        }

        HashMap<Long, Long> cache = new HashMap<>();
        println(dfs(n, adj, new HashSet<>(), cache));
    }
}