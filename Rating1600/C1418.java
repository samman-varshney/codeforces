...
public class C1418 {
    ...
    public static void helper() throws IOException {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            int skips1 = nums[i] + (i + 2 < n ? dp[i + 2] : 0);
            int skips2 = nums[i] + (i + 1 < n ? nums[i + 1] : 0) + (i + 3 < n ? dp[i + 3] : 0);
            int skips3 = nums[i] + (i + 3 < n ? dp[i + 3] : 0);
            int skips4 = nums[i] + (i + 1 < n ? nums[i + 1] : 0) + (i + 4 < n ? dp[i + 4] : 0);
