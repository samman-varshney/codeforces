...
public class LCA {
    ...
    static void build(int v, int l, int r) {
        if (l == r) {
            ...
            build(2 * v, l, mid);
            build(2 * v + 1, mid + 1, r);
            t[v] = depth[t[2 * v]] < depth[t[2 * v + 1]] ? t[2 * v] : t[2 * v + 1];
        }
    }
    ...
    static int query(int v, int tl, int tr, int l, int r) {
        if (tl >= l && tr <= r) {
            ...
            int right = query(2 * v + 1, mid + 1, tr, l, r);
            return depth[left] < depth[right] ? left : right;
        }
    }

    public static void helper() throws IOException {
        treval = new ArrayList<>();
        start = new int[n];
        depth = new int[n + 1];
        depth[n] = Integer.MAX_VALUE;
        dfs(0);
        t = new int[4 * n];
        build(1, 0, n - 1);

        for (int[] qt : queries) {
            int l = Math.min(start[qt[0]], start[qt[1]]);
            int r = Math.max(start[qt[0]], start[qt[1]]);
            println(query(1, 0, n - 1, l, r) + 1);
        }
    }
}