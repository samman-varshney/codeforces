...
public class C2184 {
    ...
    public static void helper(long n, long k) throws IOException {
        HashMap<Long, Long> map = new HashMap<>();
        long res = minDivisions(n, k, map);
        if(res > (int)1e9){
            println(-1);
        }else{
            println(res);
    ...
    public static long minDivisions(long n, long k, HashMap<Long, Long> map){
        if(n == k){
            return 0;
        }
        if(n < k){
        ...
        long left = minDivisions(n/2, k, map);
        long right = minDivisions((n+1)/2, k, map);

        long res = Math.min(left, right)+1;
        map.put(n, res);
        return res;
    }
}