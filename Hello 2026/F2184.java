...
public class F2184 {
    ...
    public static void helper(int n, int[][] edges) throws IOException {
        ...
        int leafs = countLeaf(1, -1, adj);
        if(leafs%3 == 0){
            println("YES");
            return;
        }

        Pair p = isPossible(1, -1, adj, leafs);

        println(p.valid?"YES":"NO");
    }
    static int countLeaf(int node, int parent, List<List<Integer>> adj){
        if(adj.get(node).size() == 1 &&
         adj.get(node).get(0) == parent){
            return 1;
        }
        int count = 0;
        for(int nbrs: adj.get(node)){
            if(nbrs != parent){
                count += countLeaf(nbrs, node, adj);
            }
        }
        return count;
    }
    static Pair isPossible(int node, int parent, List<List<Integer>> adj, int total){
        if(adj.get(node).size() == 1 &&
         adj.get(node).get(0) == parent){
        ...
        for(int nbrs: adj.get(node)){
            if(nbrs != parent){
                Pair p = isPossible(nbrs, node, adj, total);
                if(p.valid){
                    return p;
                }
                leafs += p.leafs;
        ...
        return new Pair(leafs, (total - leafs + 1)%3 == 0);
    }
}
class Pair{
    int leafs;
    boolean valid;
    Pair(int leafs, boolean valid){
        this.leafs = leafs;
        this.valid = valid;
    }
}