...
public class C2166 {
    ...
    public static void helper(int n, int[] nums) throws IOException {
        ...
        Arrays.sort(indices, (a, b)->(a[1]-b[1]));
        ...
        for(int i=0; i<n-1; i++){
            int idx = indices[i][0];
            if(left.get(idx) > right.get(idx)){
                cost += nums[right.get(idx)];
            }else{
                cost += nums[left.get(idx)];
            }
            left.put(left.get(idx), right.get(idx));
            right.put(right.get(idx), left.get(idx));
        }
        println(cost);
    }
}
