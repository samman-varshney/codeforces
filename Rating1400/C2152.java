...
public class C2152 {
    ...
    public static void main(String[] args) {
        try {
            ...
            while (tcase-- > 0) {
                ...
                int[][] query = read2DArray(x[1], 2);

                helper(x[0], x[1], a, query);
            }
        } catch (Exception err) {
    ...
    public static void helper(int n, int q, int[] a, int[][] queries) throws IOException {
        ...
        for(int i=1; i<=n; i++){
            ...
            dp[i] = dp[i-1] + (i>1 && a[i-2]!=a[i-1]?1:0);
        }
        ...
        for(int[] query: queries){
            ...
            int ones = prefixOne[r] - prefixOne[l-1];
            int zeros = prefixZero[r] - prefixZero[l-1];
