...
public class A698 {
    ...
    public static int solve(int i, int prev, int[] nums, int[][] dp){
        ...
        int rest = solve(i+1, 0, nums, dp)+1;
        int contest = 1000000;
        if((nums[i] == 1 || nums[i] == 3)&&prev != 1){
            contest = solve(i+1, 1, nums, dp);
        }
        int gym = 1000000;
        if((nums[i] == 2 || nums[i] == 3) && prev != 2){
            gym = solve(i+1, 2, nums, dp);
        }

        return dp[i][prev] = min(rest, min(contest, gym));
    }
}
