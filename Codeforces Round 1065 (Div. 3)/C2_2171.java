...
public class C2_2171 {
    ...
    public static void helper(int n, int[] a, int[] b) throws IOException {
        int[] bits = new int[32];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < 32; j++) {
                bits[j] += (b[i] >> j) & 1;
        ...
        // check if count for all the bits is even
        int msb = -1;
        for (int i = 31; i >= 0; i--) {
            int x = bits[i];
            if (x % 2 != 0) {
        ...
        // find the right most index where the msb is different for a and b
        int idx = -1;
        for (int i = n - 1; i >= 0; i--) {
