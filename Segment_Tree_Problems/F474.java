...
public class F474 {
    ...
    public static void main(String[] args) {
        try {
            ...
            while (tcase-- > 0) {
                ...
                int m = readInt();
                int[][] queries = read2DArray(m, 2);
                helper(n, m, nums, queries);
            }
        } catch (Exception err) {
    ...
    static int[] min, count, gcd;

    public static void helper(int n, int m, int[] nums, int[][] queries) throws IOException {
        min = new int[4 * n];
        count = new int[4 * n];
        gcd = new int[4 * n];
        build(1, 0, n - 1, nums);
        ...
        for (int[] q : queries) {
            int queen = queryGCD(1, 0, n - 1, q[0] - 1, q[1] - 1);
            int survivor = queryCount(1, 0, n - 1, q[0] - 1, q[1] - 1, queen);
            System.out.println(q[1] - q[0] - survivor + 1);
        }
    }

    static void build(int v, int tl, int tr, int[] nums) {
        if (tl == tr) {
            gcd[v] = nums[tl];
            min[v] = nums[tl];
            count[v] = 1;
            ...
            gcd[v] = getGCD(gcd[2 * v], gcd[2 * v + 1]);
            min[v] = Math.min(min[2 * v], min[2 * v + 1]);
            count[v] = (min[v] == min[2 * v] ? count[2 * v] : 0) + (min[v] == min[2 * v + 1] ? count[2 * v + 1] : 0);
        }
    }

    static int queryGCD(int v, int tl, int tr, int l, int r) {
        if (tl >= l && tr <= r) {
            return gcd[v];
        } else if (tl > r || tr < l) {
            return 0;
            ...
            int mid = (tl + tr) / 2;
            int left = queryGCD(2 * v, tl, mid, l, r);
            int right = queryGCD(2 * v + 1, mid + 1, tr, l, r);

            return getGCD(left, right);
        }
    }

    static int queryCount(int v, int tl, int tr, int l, int r, int val) {
        if (tl >= l && tr <= r) {
            if (min[v] == val)
                return count[v];
            return 0;
            ...
            int mid = (tl + tr) / 2;
            return queryCount(2 * v, tl, mid, l, r, val) + queryCount(2 * v + 1, mid + 1, tr, l, r, val);
        }
    }
