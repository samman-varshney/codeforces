...
public class QTREE3 {
    ...
    static void update(int v, int l, int r, int idx, int val) {
        if (l == idx && r == idx) {
            t[v] = state[val] == 0 ? Integer.MAX_VALUE : val;
        } else {
            int mid = (l + r) / 2;
            if (idx <= mid) {
    ...
    static int query(int v, int tl, int tr, int l, int r) {
        if (tl >= l && tr <= r) {
            ...
            int mid = (tl + tr) / 2;
            int left = query(2 * v, tl, mid, l, r);
            return left != Integer.MAX_VALUE ? left : query(2 * v + 1, mid + 1, tr, l, r);
        }
    }

    static int getFirstBlack(int a, int b) {
        ...
        while (head[a] != head[b]) {
            ...
            int res = query(1, 0, n, pos[head[a]], pos[a]);
        ...
        int res = query(1, 0, n, pos[b], pos[a]);
        if (res != Integer.MAX_VALUE) {
            black = res;
        }
        return black;
    ...
    public static void helper() throws IOException {
        ...
        for (int i = 0; i < q; i++) {
            ...
            if (type == 0) {
                state[node] ^= 1;
                update(1, 0, n, pos[node], node);
            } else {
                int res = getFirstBlack(node, 1);
