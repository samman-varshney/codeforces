...
public class PathQuery {
    ...
    // head is the array holding the master of the chain
    // heavy is holding the heavy child of a node, in case of leaf node it is zero
    static int[] heavy, depth;
    ...
    static void dfsHLD(int u, int chain) {
        ...
        // move to the heavy first then cover the light childs
        if (heavy[u] != 0) {
    ...
    static long getSum(int a, int b) {
        long sum = 0;
        while (head[a] != head[b]) {
            // the one whose head is at higher depth needs to be raised hence we
            // shift the head to a chain just above it
            if (depth[head[a]] < depth[head[b]]) {
                int t = a;
                a = b;
                b = t;
            }
            sum += query(1, 0, n, pos[head[a]], pos[a]);
            a = parent[head[a]];
        }
        ...
        sum += query(1, 0, n, pos[b], pos[a]);
        // since now both the nodes are on the same chain,
        // the one which is at lower depth is the LCA between them
        return sum;
    }
