...
public class E1304 {
    ...
    static int getKthNode(int a, int k) {
    ...
    static String query(int x, int y, int a, int b, int k) {
        int lca = getLCA(a, b);
        ...
        lca = getLCA(a, x);
        dist = depth[a] + depth[x] - 2 * depth[lca];
        lca = getLCA(b, y);
        dist += depth[b] + depth[y] - 2 * depth[lca] + 1;
        ...
        lca = getLCA(a, y);
        dist = depth[a] + depth[y] - 2 * depth[lca];
        lca = getLCA(b, x);
        dist += depth[b] + depth[x] - 2 * depth[lca] + 1;
        if (dist <= k && (k - dist) % 2 == 0) {
            return "YES";
        }
    ...
    public static void helper() throws IOException {
        ...
        for (int[] q : queries) {
            println(query(q[0], q[1], q[2], q[3], q[4]));
        }
        bw.flush();
    }
}