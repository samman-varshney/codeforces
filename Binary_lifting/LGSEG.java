...
public class LGSEG {
    ...
    static void dfs(int u, long prev) {
        long curr = prev + v2[u];
        nodes[u] = curr;
        for (int v : adj.get(u)) {
            if (v != u)
    ...
    static void build(int[][] t, int[] p) {
        t = new int[limit + 1][n + 1];
        t[0] = p;
    ...
    public static void helper() throws IOException {
        ...
        long res = 0;
        for (int i = 0; i <= n; i++) {
            int p = getKthAncestor(i, k);
            res = Math.max(res, nodes[i] - nodes[p]);
        }
