...
public class B1142 {
    ...
    static void buildTree(int v, int l, int r) {
        if (l == r) {
            t[v] = lt[l];
        } else {
            int mid = (l + r) / 2;
    ...
    public static void helper() throws IOException {
        // NEW CODE:
        parent = new int[m + 1];
        parent[m] = m; // The sentinel node must point to itself
        int[] pos = new int[n + 1];
        Arrays.fill(pos, m);
        for (int i = m - 1; i >= 0; i--) {
            int val = a[i];
            int ancestor = p[(indices[val] + 1) % n];
            parent[i] = pos[ancestor];
            pos[val] = i;
        }
        ...
        table = new int[limit + 1][m + 1];
        build();
        lt = new int[m];
        for (int i = 0; i < m; i++) {
            lt[i] = getKthAncestor(i, n - 1);
        }
        ...
        for (int[] qt : queries) {
            ...
            int idx = query(1, 0, m - 1, qt[0] - 1, qt[1] - 1);
            s.append(idx <= qt[1] - 1 ? 1 : 0);
        }
