...
public class D1516 {
    ...
    static int getBlocks(int l, int r) {
        int blocks = 0;
    ...
    public static void helper() throws IOException {
        ...
        parent[n] = n;
        for (int i = n - 1; i >= 0; i--) {
            parent[i] = min(getAncestor(i, nums[i]), parent[i + 1]);
        }
        ...
        for (int[] q : queries) {
            int l = q[0] - 1;
            int r = q[1] - 1;
            println(getBlocks(l, r) + 1);
        }

        bw.flush();
    }

    static int getAncestor(int idx, int a) {
        int ancestor = n;

        if (a % 2 == 0) {
            if (map.containsKey(2)) {
                ancestor = min(ancestor, map.get(2));
            }
            map.put(2, min(map.getOrDefault(2, n), idx));
            while (a % 2 == 0) {
                a /= 2;
            }
        }
        ...
        for (int i = 3; i * i <= a; i += 2) {
            if (a % i == 0) {
                if (map.containsKey(i)) {
                    ancestor = min(ancestor, map.get(i));
                }
                map.put(i, min(map.getOrDefault(i, n), idx));
                while (a % i == 0) {
                    a /= i;
                }
            }
        ...
        if (a > 1) {
            if (map.containsKey(a)) {
                ancestor = min(ancestor, map.get(a));
            }
            map.put(a, min(map.getOrDefault(a, n), idx));
        }
