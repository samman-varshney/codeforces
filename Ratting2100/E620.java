...
public class E620 {
    ...
    static void update(int v, int tl, int tr, int l, int r, int color) {
        if (tl >= l && tr <= r) {
            t[v] = 1L << color;
            lazy[v] = color;
        } else if (tl > r || tr < l) {
            ...
            push(v);
            int mid = (tl + tr) / 2;
            update(2 * v, tl, mid, l, r, color);
            update(2 * v + 1, mid + 1, tr, l, r, color);
    ...
    static long query(int v, int tl, int tr, int l, int r) {
        if (tl >= l && tr <= r) {
            ...
            push(v);
            int mid = (tl + tr) / 2;
            long left = query(2 * v, tl, mid, l, r);
            long right = query(2 * v + 1, mid + 1, tr, l, r);
            return left | right;
        }
    }
    ...
    public static void helper() throws IOException {
        ...
        for (List<Integer> query : queries) {
            if (query.get(0) == 1) {
                int l = start[query.get(1) - 1];
                int r = end[query.get(1) - 1];
                int color = query.get(2);
                update(1, 0, n - 1, l, r, color);
            } else {
                int l = start[query.get(1) - 1];
                int r = end[query.get(1) - 1];
                println(count(query(1, 0, n - 1, l, r)));
            }
        }
    }
}