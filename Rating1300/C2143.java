...
public class C2143 {
    ...
    public static void helper(int n, int[][] edges) throws IOException {
        ...
        for (int[] ed : edges) {
            int u = ed[0] - 1, v = ed[1] - 1, x = ed[2], y = ed[3];
            adj.get(u).add(new Triplet(v, x, y));
            adj.get(v).add(new Triplet(u, y, x));
        }
    ...
    static void dfs(int node, int parent, List<List<Triplet>> adj, int[][] level) {
        for (Triplet t : adj.get(node)) {
            if (t.node != parent) {
                level[t.node][1] = level[node][1] + (t.x < t.y ? 1 : -1);
                dfs(t.node, node, adj, level);
            }
        }
    }
}
