...
public class D1968 {
    ...
    public static void main(String[] args) {
        try {
            int tcase = readInt();
            while (tcase-- > 0) {
                int[] x = readIntArray(4);
                int[] p = readIntArray(x[0]);
                int[] a = readIntArray(x[0]);
                helper(x[0], x[1], x[2], x[3], p, a);
            }
        } catch (Exception err) {
    ...
    public static void helper(int n, int k, int pb, int ps, int[] p, int[] a) throws IOException {
        for (int i = 0; i < n; i++) {
            p[i]--;
        }
        long shasha = getMaxScore(a, p, ps - 1, k);
        long bodya = getMaxScore(a, p, pb - 1, k);

        if (shasha == bodya) {
            System.out.println("Draw");
        } else if (shasha > bodya) {
    ...
    static long getMaxScore(int[] a, int[] p, int pos, int k) {
        long maxScore = 0;
        long score = 0;
        int limit = Math.min(a.length, k);
        for (int i = 0; i < limit; i++) {
            k--;
            score += a[pos];
            maxScore = max(score + (long) k * a[pos], maxScore);
            if (pos == p[pos])
                break;
            pos = p[pos];
        }
