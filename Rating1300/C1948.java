...
public class C1948 {
    ...
    public static void main(String[] args) {
        try {
            ...
            while (tcase-- > 0) {
                ...
                grid[0] = readString().toCharArray();
                grid[1] = readString().toCharArray();
                helper(n, grid);
            }
        } catch (Exception err) {
    ...
    public static void helper(int n, char[][] grid) throws IOException {
        int[][][] visited = new int[2][n][2];
        System.out.println(dfs(0, 0, 0, visited, grid) ? "YES" : "NO");
    }

    static int[][] D = { { -1, 0 }, { 1, 0 }, { 0, 1 }, { 0, -1 } };

    static boolean dfs(int x, int y, int will, int[][][] visited, char[][] grid) {
        int n = 2, m = grid[0].length;

        if (x == n - 1 && y == m - 1 && will == 0) {
            return true;
        }
        ...
        if (will == 0) {
            for (int[] d : D) {
                ...
                if (row >= 0 && row < n && col >= 0 && col < m && visited[row][col][1] == 0) {
                    if (dfs(row, col, 1, visited, grid))
                        return true;
                }
            }
        } else {
            int row = x;
            int col = y + (grid[x][y] == '<' ? -1 : +1);

            if (visited[row][col][0] == 0 && dfs(x, y + (grid[x][y] == '<' ? -1 : +1), 0, visited, grid))
                return true;
        }
