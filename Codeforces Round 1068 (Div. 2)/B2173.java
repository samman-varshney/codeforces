...
public class B2173 {
    ...
    public static void helper(int n, long[] a, long[] b) throws IOException {
        long[][] prefix = new long[n][2];
        
        for(int i=1; i<n; i++){
            long a1 = Math.abs(prefix[i-1][0] - a[i-1]), a2 = Math.abs(prefix[i-1][1] - a[i-1]);
            if(a1 > a2){
                prefix[i][0] = prefix[i-1][0] - a[i-1];
            }else{
                prefix[i][0] = prefix[i-1][1] - a[i-1];
            }
            long b1 = Math.abs(-prefix[i-1][0] + b[i-1]), b2 = Math.abs(-prefix[i-1][1] + b[i-1]);
            if(b1 > b2){
                prefix[i][1] = -prefix[i-1][0] + b[i-1];
            }else{
                prefix[i][1] = -prefix[i-1][1] + b[i-1];
            }
        }

        long a1 = prefix[n-1][0] - a[n-1], a2 = prefix[n-1][1] - a[n-1];
        long b1 = -prefix[n-1][0] + b[n-1], b2 = -prefix[n-1][1] + b[n-1];

        println(max(max(b1, b2), max(a1, a2)));
